	...
	public void run() {
		...
		try {
			...
			if (state == AudioRecord.STATE_INITIALIZED) {
				...
				handler.obtainMessage(TSConfig.MSG_RECORDING_START)
						.sendToTarget();
				boolean flag = true;
				...
				handler.sendEmptyMessage(TSConfig.MSG_RECORDING_STOP);
				audioRecord.stop();

			} else {

				handler.sendEmptyMessage(TSConfig.MSG_RECORDING_STATE_ERROR);

			}

		} catch (Exception e) {

			handler.sendEmptyMessage(TSConfig.MSG_RECORDING_EXCEPTION);
			...
			try {
				...
				handler.sendEmptyMessage(TSConfig.MSG_RECORDING_RELEASE);
