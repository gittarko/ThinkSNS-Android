...
public class MessageNotifier {
     ...
     * @param message
     */
    public synchronized void onNewMsg(ModelChatMessage message) {
        // 判断app是否在后台
...
//        viberateAndPlayTone(message);
    ...
    public synchronized void onNewMesg(List<ModelChatMessage> messages) {
        // 判断app是否在后台
        if (!isAppRunningForeground(appContext)) {
            Log.d(TAG, "app is running in backgroud");
     ...
     * @param isForeground
     */
    protected void sendNotification (List<ModelChatMessage> messages, 
    		boolean isForeground){
        for(ModelChatMessage message : messages){
            if(!isForeground){
                notificationNum++;
                fromUsers.add(message.getFrom_uname());
            }
        }
        sendNotification(messages.get(messages.size()-1), isForeground, false);
    }
    
    protected void sendNotification (ModelChatMessage message, boolean isForeground){
     ...
     * @param message
     */
    protected void sendNotification(ModelChatMessage message, boolean isForeground, boolean numIncrease) {
...
//            }
     ...
     * 手机震动和声音提示
     */
    public void viberateAndPlayTone(ModelChatMessage message) {
        if(message != null){
...
//                			appContext.getPackageName() + "/" + R.raw.app_notify);
    ...
    public interface NotificationInfoProvider {
         ...
         * @return null为使用默认
         */
        String getDisplayedText(ModelChatMessage message);
         ...
         * @return null为使用默认
         */
        String getLatestText(ModelChatMessage message, int fromUsersNum, int messageNum);
         ...
         * @return null为使用默认
         */
        String getTitle(ModelChatMessage message);
         ...
         * @return 0使用默认图标
         */
        int getSmallIcon(ModelChatMessage message);
         ...
         * @return null为使用默认
         */
        Intent getLaunchIntent(ModelChatMessage message);
    }
}
