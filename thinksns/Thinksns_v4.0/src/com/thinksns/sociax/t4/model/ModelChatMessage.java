...
public class ModelChatMessage extends SociaxItem {
	private String chat_user_face, chat_user_name;// 聊天对方的姓名/头像url，
	...
	public void setPoi_name(String poi_name) {
		...
		Log.v("ModelChatMessage setPoiNanme", "wztest " + poi_name);
	}
...
//		this.setUid_loginUser(Thinksns.getMy().getUid());// 默认发起人是本人
//		this.setFrom_uname(Thinksns.getMy().getUserName());
//		this.setFrom_uface(Thinksns.getMy().getUserface());
	}
	...
	public ModelChatMessage(JSONObject data) {
		try {
			...
			Log.v("getMessage", "---model----getMessage json---------"+data.toString());
			
			this.setUid_loginUser(Thinksns.getMy().getUid());// 默认发起人是本人
			if (data.has("to_uid") && data.has("from_uid")) {// 设置和谁的聊天，如果to_uid是我，则from_uid是对方，否则to_uid是对方
				this.setUid_chatUser(data.getInt("to_uid") == Thinksns.getMy()
	 ...
	 * 
	 * @param msgtype
	 *            =text/link/position/card/image/voice
	 *            特殊类型msgtype=textrecord作为清理历史记录之后遗留的一条用于记录聊天信息的消息
	 */
...
//		map.put("room_info", getRoom_info());
		...
		// map.put("loginUser_id", getUid_loginUser());
		// map.put("chatUser_id ", getUid_chatUser());
		// map.put("chat_user_face", getChatUserFace());
		// map.put("chat_user_name", getChatUserName());
		// map.put("from_client_id", getFrom_client_id());
		// map.put("to_client_id", getTo_client_id());
		// map.put("min_max", getMin_max());
		// map.put("from_uname", getFrom_uname());
		// map.put("fromFace", getFrom_uface());
		// map.put("to_uid", getTo_uid());
		// map.put("to_uname", getTo_uname());
		// map.put("toFace", getTo_uface());
		// map.put("imageUrl", getImageUrl());
		// map.put("voice_url", getVoiceUrl());
		// map.put("poi_name", getPoi_name());
		// map.put("poi_image", getPoi_image());
		// map.put("members", getMembers());
		// map.put("room_type", getRoom_type());
		// map.put("room_title", getRoom_title());
...
//			map.put("type", getType());
		...
		if (getMsgtype() != null)
			map.put("message_type", getMsgtype());
		if (getNotify_type() != null)
			map.put("notify_type", getNotify_type());
//		if (getRoom_info() != null)
//			map.put("room_info", getRoom_info());
		if (getQuit_uid() != 0)
			map.put("quit_uid", getQuit_uid());
		...
		// map.put("from_uid", getUid_loginUser() + "");
		// if (getTo_uname() != null)
		// map.put("to_uname", getTo_uname());
		 if (getTo_uid() != 0)
			 map.put("to_uid", getTo_uid() + "");
		...
		try {
			...
			if (getMsgtype() != null) {
				if (getMsgtype().equals("image")) {
					...
					// json.accumulate("image_url", getImageUrl());
					// json.accumulate("image_width", getImage_width());
					// json.accumulate("image_height", getImage_height());
				} else if (getMsgtype().equals("voice")) {
					json.accumulate("attach_id", getAttach_id());
					// json.accumulate("voice_url", getVoiceUrl());
					...
					// json.accumulate("poi_image", getPoi_image());
					json.accumulate("attach_id", getAttach_id());
				} else if (getMsgtype().equals("card")) {
					json.accumulate("uid", getCard_uid());
					// json.accumulate("card_uname", getCard_uname());
					// json.accumulate("card_info", getCard_intro());
					// json.accumulate("card_avatar", getCard_avatar());
				}
			}
